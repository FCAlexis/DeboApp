...
import { Person, Purchase, Installment, Payment } from '../shared/models/debt.model';
...
export class DebtService {
  ...
  async registerPayment(personId: string, amountCents: number): Promise<void> {
    ...
    // 1. Obtener deudas actuales para el motor
    const allInstallments = await this.db.getAll<Installment>('installments');
    const allPayments = await this.db.getAll<Payment>('payments');
    ...
    const debtItems = allInstallments
      ...
      .map(i => ({
        ...
        createdAt: new Date(), // Simplificado para el ejemplo
        montoTotalCentavos: i.amountCents,
        montoPagadoCentavos: 0 // En un sistema real, sumaríamos las allocations existentes
      }));

    // 2. Ejecutar Motor de Distribución
    const result = PaymentEngine.distribuirPago(personId, amountCents, debtItems);

    // 3. Guardar el pago y las asignaciones (si implementamos la tabla allocations)
