...
const componentPath = join(__dirname, 'persons.component.ts');

describe('Persons Component — Responsive (RF08, RF03)', () => {
  let source: string;
  ...
  /* ── 4.3 Touch targets ── */
  it('should set .delete-btn to min 44x44px on mobile (RF03)', () => {
    expect(source).toMatch(/@media[\s\S]*delete-btn[\s\S]*min-width\s*:\s*44px/);
    expect(source).toMatch(/@media[\s\S]*delete-btn[\s\S]*min-height\s*:\s*44px/);
  });
