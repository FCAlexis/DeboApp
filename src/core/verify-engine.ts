import { PaymentEngine, DebtItem } from './PaymentEngine.ts';

function createDebt(id: string, tipo: 'AJUSTE' | 'CUOTA', total: number, pagado: number, vence: 'ayer' | 'hace2' | 'futuro' | 'null', created: number = Date.now()): DebtItem {
  const now = new Date();
...
console.log("🚀 Ejecutando Suite de Tests: Motor de Distribución de Pagos\n");
